...
class AttendanceApp:
    def __init__(self):
        ...
        self.root.title("Davomat Tizimi (CUDA Optimallashtirilgan)")
        ...
        # CUDA GPU xotirasini ishlatish uchun
        self.gpu_frame = None
    ...
    def create_main_interface(self):
        ...
        ttk.Label(self.current_frame, text="Davomat Tizimi (CUDA)", 
    ...
    def save_to_database(self):
        ...
        for file_path in file_paths:
            ...
            if image is None:
                messagebox.showwarning("Ogohlantirish", f"{file_path} faylini o'qib bo'lmadi.")
                continue
            ...
            for i in range(detections.shape[2]):
                ...
                if confidence > 0.5:
                    ...
                    descriptor = face_resized.flatten().astype(np.float32)
    ...
    def detect_available_cameras(self, max_index=3):
        ...
        for i in range(max_index):
            try:
                cap = cv2.VideoCapture(i, cv2.CAP_DSHOW)  # Windows uchun CAP_DSHOW ishlatiladi
                if cap.isOpened():
    ...
    def attendance_system(self, camera_source):
        ...
        self.running = True
        self.cap = cv2.VideoCapture(camera_source, cv2.CAP_DSHOW)  # Windows uchun CAP_DSHOW
        
        if not self.cap.isOpened():
            messagebox.showerror("Xato", f"Kamera ochilmadi! Indeks: {camera_source}. Iltimos, kamera ulanishini tekshiring.")
            self.show_attendance_section()
            return
        ...
        ttk.Label(self.current_frame, text="Davomat Davom Etmoqda (CUDA)...",
                 font=("Helvetica", 16, "bold")).pack(pady=10)
        ...
        def video_loop():
            while self.running and not self.video_event.is_set():
                ...
                # CUDA GPU-ga tasvirni yuklash
                self.gpu_frame = cv2.cuda_GpuMat()
                self.gpu_frame.upload(frame)
                
                # Tasvirni CPU-ga qaytarib olish (agar kerak bo'lsa)
                ...
                for i in range(detections.shape[2]):
                    ...
                    if confidence > 0.5:
                        ...
                        face_resized = cv2.resize(face, (128, 128))
                        face_descriptor = face_resized.flatten().astype(np.float32)
                        ...
                        for person_name, person_data in self.database.items():
                            distances = []
                            for stored_descriptor in person_data["descriptors"]:
                                distance = np.linalg.norm(face_descriptor - stored_descriptor)
                                distances.append(distance)
                            if distances:
                                min_distance = np.min(distances)
                                probability = 1 - (min_distance / np.max(distances)) if np.max(distances) > 0 else 0.5
